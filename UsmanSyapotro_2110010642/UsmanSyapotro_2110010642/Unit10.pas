...
if e_1.Text='' then
  begin
    ...
    if e_2.Text=''then
    begin
     ShowMessage('NAMA PERUMAHAN BELUM DIISI DENGAN BENAR');
    end else
    if e_3.text=''then
    begin
     ShowMessage('NAMA DEVELOPER DIISI DENGAN BENAR');
    end else
    if e_4.text=''then
    begin
    ShowMessage('NAMA DIREKTUR BELUM SESUAI');
    end else
    if e_5.text=''then
    begin
     ShowMessage('KONTAK PEMASARAN BELUM SESUAI');
     end else
    if e_6.text=''then
    begin
     ShowMessage('KONTAK PERUSAHAAN BELUM SESUAI');
     end else
    if e_7.text=''then
    begin
     ShowMessage('ALAMAT KANTOR DIISI DENGAN BENAR');
    end else
    if e_8.text=''then
    begin
 ...
 zqry1.SQL.Clear;
zqry1.SQL.Add('insert into tbl_profil_perumahan values("'+e_1.Text+'","'+e_2.Text+'","'+e_3.Text+'","'+e_4.Text+'","'+e_5.Text+'","'+e_6.Text+'","'+e_7.Text+'","'+e_8.Text+'")');
 zqry1.ExecSQL ;

 zqry1.SQL.Clear;
 zqry1.SQL.Add('select * from tbl_profil_perumahan');
 zqry1.Open;
...
zqry1.SQL.Clear;
zqry1.SQL.Add('Update tbl_profil_perumahan set nm_perumahan="'+e_2.Text+'",nm_developer="'+e_3.Text+'",nm_direktur="'+e_4.Text+'",kontak_pemasaran="'+e_5.Text+'",kontak_perusahaan="'+e_6.Text+'",alamat_kantor= "'+e_7.Text+'",deskripsi="'+e_8.Text+'" where id="'+id+'"');
zqry1. ExecSQL;
...
begin
zqry1.SQL.Clear;
zqry1.SQL.Add(' delete from tbl_profil_perumahan where id="'+id+'"');
zqry1. ExecSQL;
zqry1.SQL.Clear;
zqry1.SQL.Add('select * from tbl_profil_perumahan');
zqry1.Open;
